...
Usage:
  python generate-thumbnail-from-box.py <slug> <box-art-url-or-path> [game-title]

Examples:
  python generate-thumbnail-from-box.py wingspan-review "https://res.cloudinary.com/..." "Wingspan"
  python generate-thumbnail-from-box.py wingspan-review "/path/to/wingspan-box.jpg" "Wingspan"
  python generate-thumbnail-from-box.py wingspan-review "https://m.media-amazon.com/images/..." "Wingspan"
...
def build_prompt(game_title, box_art_path):
...
def main():
    ...
    game_title  = sys.argv[3] if len(sys.argv) > 3 else slug.replace('-review', '').replace('-', ' ').title()
    ...
    prompt = build_prompt(game_title, box_art_local)
